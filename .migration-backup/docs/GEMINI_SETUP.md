...
2026-09-14 후속 사용자 결정: 지금은 AI·서버 모두 무료 범위만 사용하고 지인에게 테스트한다. 사용자가 지어낸 개인정보를 넣은 가상 자료를 제공할 예정이며 아직 받거나 전송하지 않았다. 서비스 시작 시 유료 전환할 계획이지만 실제 금액·설정 변경은 그 전에 별도로 확인한다. **아래 CLI는 여전히 코드로 만든 2장만 처리하며, 새 결정만으로 사용자 파일 입력이 추가되거나 실제 개인정보 전송이 허용되지 않는다.** 자료 수신 후 혼입 검사와 별도 제한 시험 경로가 필요하다. [확정한 정책](DEVELOPMENT_PLAN.md#7-사용자-확정-정책과-남은-운영-설정).
