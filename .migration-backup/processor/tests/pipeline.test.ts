...
import { loadConfig, type ProcessorConfig } from "../src/config.js";
import { DELETION_PENDING_ACTIVE } from "../src/asset-lifecycle.js";
import {
  isPercent,
...
import { finalizeGuideDeletion } from "../src/asset-lifecycle.js";
import { JsonGuideRepository } from "../src/repository.js";
import { LocalStorage, type Storage } from "../src/storage.js";

const localRequire = createRequire(import.meta.url);

function requiredExecutable(value: unknown, name: string): string {
  if (typeof value !== "string" || value.length === 0) {
    throw new Error(`Bundled ${name} binary is required for pipeline tests.`);
  }
  return value;
}

const ffmpegPath = requiredExecutable(localRequire("ffmpeg-static"), "ffmpeg");
const ffprobePath = requiredExecutable(
  (localRequire("ffprobe-static") as { path?: unknown }).path,
  "ffprobe",
);
