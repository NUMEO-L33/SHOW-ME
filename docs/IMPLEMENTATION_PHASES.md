...
전체 순서와 현재 착수 지점은 [전체 개발 계획 v1](DEVELOPMENT_PLAN.md), 데이터 계약·실패 처리·완료 기준은 [기능별 상세 설계](NEXT_PHASE_DESIGN.md), AI 큐·비용 예약은 [실행 설계](ANALYSIS_EXECUTION_DESIGN.md)가 기준입니다. Gate 2 실기기 확인은 전체 계획 A0로 남기고, Gate 3–7과 후속 모바일 제작 화면을 각각 구현·검증·리뷰 단위로 진행합니다. 설계 문서의 제안은 구현 완료를 뜻하지 않습니다.
