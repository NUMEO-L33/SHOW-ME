...
진행 상황: **3A 계약·저장 기반 완료, 3B-1 Gemini 어댑터와 가상 화면 실제 연결 확인 완료**. 입력/출력 검증, 제한된 재시도·타임아웃, 로컬 시험용 하루 10회 예약 한도를 유지한다. 기본 시험은 외부 통신을 하지 않으며 실제 호출은 키와 무료 등급 확인·가상 화면 전송 동의가 필요하다. `gemini-3.8-flash` 기본값은 그대로이고, 사용자가 승인한 `gemini-3.5-flash-lite`를 명시적으로 선택해 시험할 수 있다. 2026-09-14에 3.5 Flash-Lite로 가상 화면 2장의 한국어 설명·버튼 좌표·검증된 결과 파일 저장까지 성공했다. 분석 API·제품 내 동의 화면·운영 예산·자동 dispatcher·PostgreSQL 실환경 경합 검증은 여전히 남아 있으므로 Gate 3 전체 완료는 아니다. [3A 기록](GATE_3A_CHECKPOINT.md), [Gemini 설정과 범위](GEMINI_SETUP.md)를 참고한다.

2026-09-14 비교 기록: 3.8 모델 정보 조회는 HTTP 200이지만 분석 2회는 HTTP 503, 이후 승인받은 3.5 Flash-Lite 분석 1회는 성공했다. 당일 총 예약은 3/10회다. 두 모델의 한도를 공유하며 자동 전환은 하지 않는다. 다음은 분석 API·동의·큐·운영 예산 연결 단계이며 아직 구현하지 않았다.
