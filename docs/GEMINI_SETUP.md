...
1. 연결 검증 완료: 3.5 Flash-Lite 시험에서 `analyzed-synthetic-only`와 결과 저장을 확인했다. 2026-09-14 시험 사용량은 3.8 실패 2회 + 3.5 성공 1회, **로컬 예약 3/10회**다. 기본 3.8 모델은 유지했으며, 후속 분석 API의 서버 소유 모델 정보는 성공한 3.5로 명시했다. 자동 재시험·모델 전환·과금 활성화는 하지 않았다.
2. Gate 3B-2A 완료: 인증된 분석 요청·조회·취소 API와 요청 본문의 동의 검증을 추가했다. **서버 시작 시 새 분석 접수는 꺼져 있으며 Google 호출은 없다.** [API 계약](ANALYSIS_API.md). 다음은 별도 작업 큐, 운영 DB 기반 요청/비용 예약, 실패 묶음 보존·취소·복구, 제품 내 외부 전송 동의 화면이다. 실제 민감정보 사용 조건은 별도 결정.
