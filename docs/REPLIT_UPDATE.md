...
2026-09-17 이미지 초기 실행 지연으로 진행이 멈췄다면 [이미지 처리 시간 제한 보완](REPLIT_IMAGE_DEADLINES.md)을 따른다. [headless 교체 이력](REPLIT_HEADLESS_MEDIA.md)도 보존한다. 일반 전체 검사를 반복해 성공한 결과로 첫 이미지 실패를 덮지 않는다.

프런트엔드의 이전 보완은 [제작 의도 입력·작업 상태 표시](REPLIT_INTENT_AND_JOB_UI.md), 이후 서버 저장 연결은 [제작 의도·편집 서버 저장](REPLIT_SERVER_DRAFT.md)을 따른다. 최신 버전은 의도와 수동 편집을 비공개 서버 초안에 저장하며 AI에는 아직 적용하지 않는다.

`5ba696d` 이후에는 [비공개 편집 자동 저장](REPLIT_AUTOSAVE.md)을 따른다. 편집을 멈추면 약 1초 뒤 저장하며, 녹화·파일 업로드를 자동 시작하지 않는다. 이번 변경은 프런트엔드만 바꾸고 서버 저장 API는 그대로 사용한다.
...
로컬 브랜치가 이미 있으면 새로 만들거나 강제로 맞추지 말고 커밋/변경 상태부터 확인한다. 현재 브랜치가 `codex/replit-migration-hardening`이고 변경 파일이 없으며 예상한 이력이라면 `git pull --ff-only`로 갱신한다. 새 HEAD는 GitHub에서 확인한 보완본 커밋과 같아야 한다.
...
Replit만 처리·저장하도록 제한하는 최신 보완과 남은 실환경 확인은 [REPLIT_RECORDING_PRIVACY.md](REPLIT_RECORDING_PRIVACY.md)를 따른다. 이번 보완은 웹/API 모두 갱신이 필요하며, 로컬 검사 통과가 실제 저장소 권한 감사나 유출 0% 보증은 아니다.
