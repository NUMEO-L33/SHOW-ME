...
import { budgetUnitFields, zeroBudgetUnits, type AnalysisAccountingCommand } from "../src/analysis-accounting-contract.js";
...
import { analysisAccountingControls, analysisBudgetWindows, analysisRequestAttempts, analysisReservations, analysisRuns, guides } from "../src/db/schema.js";
...
test("v4 upgrades add only zero release fields, preserve reads, and reject corrupted v5 closure records", async (context) => {
  ...
  assert.deepEqual(await h.state(), old); await h.close(nextDay); const saved = await h.state(); assert.equal(saved.version, 5);
  for (const mutate of [
...
test("closure migration adds safe defaults without deleting, resetting or moving historical accounting", async () => {
  const sql = await readFile("processor/drizzle/0007_analysis_reservation_closure.sql", "utf8");
