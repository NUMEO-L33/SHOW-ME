...
`0003_analysis_funding.sql`은 분석 묶음·예산 구간·예약 테이블을 추가합니다. PostgreSQL에서 새 저장소 코드를 사용하기 전에 이 migration까지 적용해야 합니다. 기존 시작 절차도 migration 성공 뒤에만 준비 상태가 됩니다. 예약 생성 이후에는 새 예산·삭제 계약을 모르는 이전 코드로 되돌리지 말고 호환 버전만 사용해야 합니다. 이번 개발에서는 실제 DB에 적용하지 않았고, 실제 PostgreSQL 경합·롤백·삭제 cascade 검증은 후속 B5에 남아 있습니다.

로컬 JSON은 형식 v2를 사용합니다. 정상 v1 자료는 읽을 수 있고 다음 저장 때 기존 가이드·초안·실행을 보존하며 v2로 승격합니다. 누락되거나 손상된 v2 예산 기록은 초기화하지 않고 실패 처리합니다. **v1 전용 예전 프로그램으로 돌아갈 수 없으므로 먼저 백업하고, 버전 숫자를 강제로 낮추거나 예산 필드를 지우지 마세요.** 기존 실행에 예산을 소급하지 않습니다. JSON 동시 쓰기 보장은 단일 저장소 인스턴스에 한정합니다.

예약 정책은 계산 전용이며 무료 사용 가능 여부나 과금 허용을 증명하지 않습니다. 취소·삭제 시 아직 정산하지 않은 최대 예약은 유지합니다. 삭제는 예약 상세와 묶음을 제거하되 내부 식별자·날짜·최대분 및 일일 집계만 남깁니다. 요청별 원장·정산은 다음 B2-B이며, 새 저장 기능은 API/dispatcher에 연결하지 않아 신규 분석 503은 유지됩니다. [범위와 검증 기록](../docs/ANALYSIS_EXECUTION_DESIGN.md)을 참고하세요.
