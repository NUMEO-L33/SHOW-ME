...
## 분석 저장·실행 호환성 — B2-A/B·B4-A·B4-B1/B2

`0003_analysis_funding.sql`은 분석 묶음·예산 구간·예약, `0004_analysis_accounting.sql`은 요청별 원장과 전체 중단 제어 행을 추가합니다. `0005_analysis_work_queue.sql`은 기존 실행의 조회 열·인덱스를, `0006_analysis_batch_completion.sql`은 묶음 상태 열과 payload 일치 제약을 추가합니다. 기존 payload에서 값을 채운 뒤 필수 조건을 적용하며 자료를 초기화하지 않습니다. **현재 PostgreSQL 저장소 코드는 `0006`까지 필요합니다.** 전체 제어 행은 migration에서 한 번만 생성하며 시작/접수 때 재생성하지 않습니다. 누락·손상은 실패 처리합니다. 기존 시작 절차도 migration 성공 뒤에만 준비 상태가 됩니다. 실행기와 저장소의 구버전/신버전 혼용을 피하고, 적용 전 백업과 호환 버전을 확보해야 합니다. 실제 DB 적용·경합·롤백·삭제 cascade 검증은 후속 B5에 남아 있습니다.

로컬 JSON은 형식 **v4**를 사용합니다. 정상 v1/v2/v3 자료는 읽을 수 있고 다음 저장 때 기존 가이드·초안·실행·예산·정산을 보존하며 v4로 승격합니다. 완료 묶음에는 검증된 결과와 사용량·전송/소유권 식별자를 저장하고 원장·최종 실행과 대조합니다. 누락·손상·버전을 낮춘 완료 자료는 초기화하지 않고 실패 처리합니다. **v1–v3 전용 예전 프로그램으로 돌아갈 수 없으므로 먼저 백업하고, 버전 숫자를 강제로 낮추거나 예산·요청·결과 필드를 지우지 마세요.** 기존 실행에 예산을 소급하지 않습니다. JSON 동시 쓰기 보장은 단일 저장소 인스턴스에 한정합니다.
...
PostgreSQL의 기본 시간은 잠금 후 조회한 DB 시계이며 명시적 시간 인자는 내부 모의 시험 전용입니다. JSON은 단일 인스턴스 직렬화만 보장합니다. 소유권 1개는 공급자에게 이미 전달된 요청까지 즉시 중단하거나 외부 실행이 항상 1개라고 보장하는 기능은 아닙니다. 종료 예산 정리·날짜를 넘는 대기/새 예산 확보는 B4-B3, 실제 전송 조건·DB·저장소 연결 검증은 B5에 남깁니다.
...
B4-B2의 `DurableAnalysisDispatcher`는 provider·신뢰하는 준비 검증기·비공개 이미지 로더를 명시적으로 주입해야 하며, 누락되면 `disabled`입니다. `start()`는 내부 주기 실행, `tick()`은 한 작업 처리, `stop()`은 중단과 제한된 숫자 정산 정리를 담당합니다. 같은 인스턴스의 중복 호출을 합치고 기본 1초 폴링, 오류 시 최대 30초 backoff를 사용합니다. 실행 180초·요청 60초·I/O 5초가 최대이며, 소유권/취소/삭제/중단 상태를 기본 1초마다 확인합니다. 완료 묶음·미전송 예약은 재사용하고 불확실 전송은 무조건 재전송하지 않습니다. 같은 살아 있는 실행에서 확인한 일부 5xx만 최대 한 번 별도 원장 차수로 재시도합니다.

이 dispatcher는 무료 전용이며 같은 예약 정책·승인 합성 입력·짧은 준비 보고서를 묶음별로 재확인합니다. Gemini 어댑터의 `single-send-v1` 계약, 내부 재시도 0, 출력 상한을 요구하고 비동기 준비 뒤 실제 fetch 직전에도 소유권/권한을 확인합니다. `executeAnalysisAccounting`에 동기식 확정 직전 검사, `failAnalysisWork`에 소유권이 일치하는 실패와 sending→uncertain 원자 저장을 추가했습니다. 현재 실환경 검증기/로더는 없고 서버 자동 시작·환경변수 스위치도 없습니다. 새 테스트 33개를 포함해 전체 336개·타입·lint·빌드 통과이며 모의 보고서/이미지/응답과 PostgreSQL 트랜잭션 대역만 사용했습니다. 실제 Google 호출·DB 적용·푸시·배포는 하지 않았습니다.
