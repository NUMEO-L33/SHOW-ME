...
예약 정책과 요청 기록은 계산 전용이며 무료 사용 가능 여부나 전송·과금 허용을 증명하지 않습니다. B2-B는 확인된 사용량을 한 번만 정산하고, 불확실 사용량은 최대 예약을 유지합니다. 상한 초과는 원장과 전체 중단 상태를 함께 저장하며 날짜 변경/재시작/삭제로 자동 해제하지 않습니다. 아직 배정하지 않은 재시도 몫은 계속 보유하고, 이미 배정된 미전송 요청은 작업 종료 확인 후에만 명시적으로 해제할 수 있습니다. 삭제는 예약 상세와 묶음을 제거하되 식별자·날짜·수치 사용 기록과 집계를 남깁니다. 삭제 뒤 늦은 정산은 거절합니다. B3에서 API 접수기를 연결했지만 실제 준비 조건 검증기와 dispatcher가 없어 기본 설정의 신규 분석 503은 유지됩니다. [정산 계약과 남은 범위](../docs/ANALYSIS_EXECUTION_DESIGN.md)를 참고하세요.

B4-A의 `listAnalysisWork`는 최대 50개(기본 20개)를 읽고, `claimAnalysisWork`는 전체 예산 제어 잠금 아래 유효한 funded 소유권을 1개로 제한합니다. lease는 최대 180초, 인수는 최대 3회이며 만료된 세 번째 인수는 실패로 종료합니다. 재시작 후 같은 저장소에서 만료 작업을 다시 발견할 수 있지만 **주기적으로 찾아 실제 처리하는 루프는 아직 연결하지 않았습니다.** 오래된 sending은 uncertain으로 바꾸고 최대 예약을 그대로 보존합니다. 한 번 소유권을 얻은 실행의 allocate/sending은 정확한 소유권 ID·횟수·유효 시간을 요구하며 늦은 수치 정산은 허용합니다.

PostgreSQL의 기본 시간은 잠금 후 조회한 DB 시계이며 명시적 시간 인자는 내부 모의 시험 전용입니다. JSON은 단일 인스턴스 직렬화만 보장합니다. 소유권 1개는 공급자에게 이미 전달된 요청까지 즉시 중단하거나 외부 실행이 항상 1개라고 보장하는 기능은 아닙니다. 실제 전송 조건·자동 실행/복구·미배정 예약 정리·날짜를 넘는 새 예산 확보는 B4-B2입니다.

B4-B1의 `completeAnalysisBatch`는 소유권·입력 지문·전송 원장을 확인해 **결과·정산·마지막 완료 초안을 한 번에 저장**합니다. 일부만 완료됐을 때는 초안을 바꾸지 않습니다. 모두 끝나도 사람이 수정한 초안은 덮어쓰지 않고 제안만 보존합니다. 잘못된 결과는 사용량을 정산하되 결과를 저장하지 않고 실패 처리하며, 사용량 상한 초과는 전체 중단 상태도 함께 저장합니다. 완료된 묶음의 추가 배정/전송을 차단하고 같은 완료 응답은 불변 기록을 반환합니다. 전체 삭제 때 결과도 제거하고 수치 원장은 남깁니다. 이 내부 기능은 아직 자동 dispatcher나 HTTP/UI에 연결하지 않았습니다.
