...
`0003_analysis_funding.sql`은 분석 묶음·예산 구간·예약, `0004_analysis_accounting.sql`은 요청별 원장과 전체 중단 제어 행을 추가합니다. `0005_analysis_work_queue.sql`은 기존 실행의 조회 열·인덱스를, `0006_analysis_batch_completion.sql`은 묶음 상태 열과 payload 일치 제약을 추가합니다. 기존 payload에서 값을 채운 뒤 필수 조건을 적용하며 자료를 초기화하지 않습니다. `0007_analysis_reservation_closure.sql`은 미배정 반환량(기본 0)·종료 시각(null)·미종료 예약 인덱스를 추가합니다. **현재 PostgreSQL 저장소 코드는 `0007`까지 필요합니다.** 전체 제어 행은 migration에서 한 번만 생성하며 시작/접수 때 재생성하지 않습니다. 누락·손상은 실패 처리합니다. 기존 시작 절차도 migration 성공 뒤에만 준비 상태가 됩니다. 실행기와 저장소의 구버전/신버전 혼용을 피하고, 적용 전 백업과 호환 버전을 확보해야 합니다. 실제 DB 적용·경합·롤백·삭제 cascade 검증은 후속 B5에 남아 있습니다.

로컬 JSON은 형식 **v5**를 사용합니다. 정상 v1/v2/v3/v4 자료는 읽을 수 있고 다음 저장 때 기존 가이드·초안·실행·예산·정산을 보존하며 v5로 승격합니다. 완료 묶음에는 검증된 결과와 사용량·전송/소유권 식별자를 저장하고 원장·최종 실행과 대조합니다. 누락·손상·버전을 낮춘 완료 자료는 초기화하지 않고 실패 처리합니다. **v1–v4 전용 예전 프로그램으로 돌아갈 수 없으므로 먼저 백업하고, 버전 숫자를 강제로 낮추거나 예산·요청·결과 필드를 지우지 마세요.** 기존 실행에 예산을 소급하지 않습니다. JSON 동시 쓰기 보장은 단일 저장소 인스턴스에 한정합니다.

예약 정책과 요청 기록은 계산 전용이며 무료 사용 가능 여부나 전송·과금 허용을 증명하지 않습니다. B2-B는 확인된 사용량을 한 번만 정산하고, 불확실 사용량은 최대 예약을 유지합니다. 상한 초과는 원장과 전체 중단 상태를 함께 저장하며 날짜 변경/재시작/삭제로 자동 해제하지 않습니다. B4-B3는 종료된 실행의 미배정 몫과 reserved를 한 번만 반환하며 sending은 uncertain으로 보존합니다. 삭제 기록도 상세를 복원하지 않고 안전한 미사용 수치만 정리합니다. 삭제는 예약 상세와 묶음을 제거하되 식별자·날짜·수치 사용 기록과 집계를 남깁니다. 삭제 뒤 늦은 정산은 거절합니다. B3에서 API 접수기를 연결했지만 실제 준비 조건 검증기와 운영 실행 연결이 없어 기본 설정의 신규 분석 503은 유지됩니다. [정산 계약과 남은 범위](../docs/ANALYSIS_EXECUTION_DESIGN.md)를 참고하세요.

B4-A의 `listAnalysisWork`는 최대 50개(기본 20개)를 읽고, `claimAnalysisWork`는 전체 예산 제어 잠금 아래 유효한 funded 소유권을 1개로 제한합니다. lease는 최대 180초, 인수는 최대 3회이며 만료된 세 번째 인수는 실패로 종료합니다. B4-B2의 내부 루프가 이 저장 기능을 사용하지만 **서버 시작이나 HTTP에는 자동 실행을 연결하지 않았습니다.** 오래된 sending은 uncertain으로 바꾸고 최대 예약을 그대로 보존합니다. 한 번 소유권을 얻은 실행의 allocate/sending은 정확한 소유권 ID·횟수·유효 시간을 요구하며 늦은 수치 정산은 허용합니다.

PostgreSQL의 기본 시간은 잠금 후 조회한 DB 시계이며 명시적 시간 인자는 내부 모의 시험 전용입니다. JSON은 단일 인스턴스 직렬화만 보장합니다. 소유권 1개는 공급자에게 이미 전달된 요청까지 즉시 중단하거나 외부 실행이 항상 1개라고 보장하는 기능은 아닙니다. 종료 정리는 B4-B3 내부 루프에 연결했습니다. 전날 queued/만료 running을 종료하고 새 요청에서 새 날짜 예산을 확보하며 자동 이월은 하지 않습니다. 실제 전송 조건·DB·저장소 검증은 B5에 남깁니다.

B4-B1의 `completeAnalysisBatch`는 소유권·입력 지문·전송 원장을 확인해 **결과·정산·마지막 완료 초안을 한 번에 저장**합니다. 일부만 완료됐을 때는 초안을 바꾸지 않습니다. 모두 끝나도 사람이 수정한 초안은 덮어쓰지 않고 제안만 보존합니다. 잘못된 결과는 사용량을 정산하되 결과를 저장하지 않고 실패 처리하며, 사용량 상한 초과는 전체 중단 상태도 함께 저장합니다. 완료된 묶음의 추가 배정/전송을 차단하고 같은 완료 응답은 불변 기록을 반환합니다. 전체 삭제 때 결과도 제거하고 수치 원장은 남깁니다. 내부 dispatcher에서 사용하지만 HTTP/UI를 활성화한 것은 아닙니다.

B4-B2의 `DurableAnalysisDispatcher`는 provider·신뢰하는 준비 검증기·비공개 이미지 로더를 명시적으로 주입해야 하며, 누락되면 `disabled`입니다. `start()`는 내부 주기 실행, `tick()`은 한 작업 처리, `stop()`은 중단과 제한된 숫자 정산 정리를 담당합니다. 같은 인스턴스의 중복 호출을 합치고 기본 1초 폴링, 오류 시 최대 30초 backoff를 사용합니다. 실행 180초·요청 60초·I/O 5초가 최대이며, 소유권/취소/삭제/중단 상태를 기본 1초마다 확인합니다. 완료 묶음·미전송 예약은 재사용하고 불확실 전송은 무조건 재전송하지 않습니다. B4-B3는 확인된 500/502/503/504 오류 번호를 저장해 재시작 후에도 최대 한 번 별도 원장 차수로 재시도합니다. 한 회에 종료 후보 20개·대기 20개를 처리하며 keyset 순환 조회로 부적격 선두 뒤 작업도 찾습니다.

이 dispatcher는 무료 전용이며 같은 예약 정책·승인 합성 입력·짧은 준비 보고서를 묶음별로 재확인합니다. Gemini 어댑터는 `locked-send-v2`, 내부 재시도 0, 출력 상한을 요구합니다. `launchAnalysisRequest`는 취소/삭제/소유권 변경과 같은 저장소 잠금 아래에서 마지막 상태를 검증하고 동기적으로 요청을 시작합니다. 응답 대기는 잠금을 풀고 하며, 이미 시작된 요청이나 응답이 유실된 전송을 다시 시작하지 않습니다. 실제 준비 검증기/비공개 로더·서버 자동 시작 연결은 없어 신규 POST 503을 유지합니다. 수정 후 340개, B4-B3 후 360개 테스트 및 타입·lint·빌드를 통과했습니다. 모두 모의 보고서/이미지/전송과 PostgreSQL 대역 검증이며 실제 Google·DB 적용·푸시·배포는 하지 않았습니다.
...
Gate 3B-2A의 분석 요청·조회·취소 API와 B3의 `DurableAnalysisAdmission`을 이 Node 서버에 연결했습니다. **기본 접수기에는 실제 준비 조건 검증기가 없으므로 새 요청은 계속 `503 ANALYSIS_UNAVAILABLE`이며 대기 작업도 만들지 않습니다.** 기존 실행 조회·취소는 인증 후 처리할 수 있습니다. 준비 조건이 모의로 충족된 테스트에서는 작업·초안·묶음·예산을 함께 저장하며 한도 초과는 `429 ANALYSIS_BUDGET_LIMIT`로 반환합니다.

기본 지출 정책은 `free_only`이며 키/시험용 `.env` 값/클라이언트 본문으로 켜거나 유료 전환할 수 없습니다. 신뢰하는 준비 조건 검증기가 승인된 가상 입력·DB·예산을 사용하는 실행기·전체 입력 토큰 상한·무료 프로젝트/할당량을 확인해야 합니다. 검증 보고서 형식 검사 자체가 실제 조건 확인은 아닙니다. 현재 그 실환경 검증기는 **미구현**이고 테스트 보고서는 가상입니다. 유료 계약은 승인 ID·프로젝트·명시된 금액 상한 일치를 검사하는 모의 시험만 했으며 실제 승인/과금 설정은 없습니다.

보고서는 최대 30초 유효하며 저장 잠금을 얻은 뒤 쓰기 직전에 만료·철회·날짜 변경·중단 신호를 다시 확인합니다. 이미 저장된 요청의 재조회는 새 허가나 예약을 만들지 않습니다. 접수는 실제 전송 허가가 아니며 B4 실행기는 전송 때 조건을 다시 검증해야 합니다. B3에는 migration/JSON 형식 변경이 없습니다. 실제 DB·실행기·제품 내 동의와 전송 검증은 후속 단계입니다. [분석 API 계약](../docs/ANALYSIS_API.md)을 참고하세요.
