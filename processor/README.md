...
## 분석 저장 호환성 — B2-A/B·B4-A

`0003_analysis_funding.sql`은 분석 묶음·예산 구간·예약, `0004_analysis_accounting.sql`은 요청별 원장과 전체 중단 제어 행을 추가합니다. B4-A의 `0005_analysis_work_queue.sql`은 기존 실행 payload에서 생성 시각·다음 인수 가능 시각·시도 횟수를 채운 후 필수 조건·일치 제약·조회 인덱스를 적용합니다. **현재 PostgreSQL 저장소 코드는 `0005`까지 필요합니다.** 전체 제어 행은 migration에서 한 번만 생성하며 시작/접수 때 재생성하지 않습니다. 누락·손상은 실패 처리합니다. 기존 시작 절차도 migration 성공 뒤에만 준비 상태가 됩니다. 실행기와 저장소의 구버전/신버전 혼용을 피하고, 적용 전 백업과 호환 버전을 확보해야 합니다. 이번 개발에서는 실제 DB에 적용하지 않았고, 실제 PostgreSQL 경합·롤백·삭제 cascade 검증은 후속 B5에 남아 있습니다.
...
PostgreSQL의 기본 시간은 잠금 후 조회한 DB 시계이며 명시적 시간 인자는 내부 모의 시험 전용입니다. JSON은 기존 v3 형식을 유지하고 단일 인스턴스 직렬화만 보장합니다. 소유권 1개는 공급자에게 이미 전달된 요청까지 즉시 중단하거나 외부 실행이 항상 1개라고 보장하는 기능은 아닙니다. 성공 묶음 저장·실제 전송 조건·자동 실행/복구·미배정 예약 정리·날짜를 넘는 새 예산 확보는 B4-B입니다.
