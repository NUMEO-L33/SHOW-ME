// Explicit synthetic acceptance check. No application DB, AI, .env or public listener.
...
export async function runPublicationStorageCheck(args: readonly string[], env: Environment,
  ...
  try {
    ...
    await h.repository.replaceSteps(runId, h.guide.steps.map(s => ({ ...s, representativeFrameKey: frame, thumbnailFrameKey: thumb })));
    const guide = (await h.repository.getGuideById(runId))!, reviewed = await reviewedAssetFixture(h.repository, guide);
    const sourceState = await h.repository.getGuideById(runId);
    ...
    runtime = new DurablePublicationRuntime({ config, repository: h.repository, storage }, { pollMs: 250, shutdownTimeoutMs: 5000 });
    const app = createProcessorApp({ config, repository: h.repository, storage, publicationAdmission: runtime.admission,
    ...
    while ((await h.repository.getPublicationJob(runId, publicationId))?.status !== "succeeded") {
      signal.throwIfAborted(); assert.notEqual((await h.repository.getPublicationJob(runId, publicationId))?.status, "failed"); await delay(100, undefined, { signal });
    }
    ...
    assert.deepEqual(await wait(readBytes(frame)), source); assert.deepEqual(await h.repository.getGuideById(runId), sourceState);
    ...
    while ((await h.repository.listPrivacyAssetBatches(runId)).length) { signal.throwIfAborted(); await delay(100, undefined, { signal }); }
    ...
    if (!pendingIO) {
      try {
        ...
        objectsRemoved = true; for (const cleanup of cleanups) await cleanup(); localRemoved = true;
      } catch { passed = false; }
    }
    pendingIO ||= inFlight.size > 0 || streams.size > 0;
    if (!localRemoved) log(`PUBLICATION_STORAGE_CHECK CLEANUP_PENDING ${h.root}`);
  }
  const result = { passed: passed && objectsRemoved && localRemoved, mode: target.mode, remoteObjectsRemoved: target.mode === "replit" && objectsRemoved,
    localFixtureRemoved: localRemoved, pendingIO, applicationDatabaseUsed: false, externalAIUsed: false, publicListener: false };
