...
import { AnalysisAccountingError, type AnalysisAccountingCommand } from "../src/processor/analysis-accounting-contract.js";
...
import { analysisAccountingControls, analysisBudgetWindows, analysisRequestAttempts, analysisReservations, guides } from "../src/processor/db/schema.js";
...
test("v2 funding upgrades without repricing and malformed current attempts or counters never reset", async (context) => {
  ...
  await h.begin(); await h.settle();
  const saved = await h.state(); assert.equal(saved.version, 5);
...
test("request accounting migration retains records, uniquely binds slots and seeds the stop control once", async () => {
  ...
  const migration = await readFile("drizzle/0004_analysis_accounting.sql", "utf8");
