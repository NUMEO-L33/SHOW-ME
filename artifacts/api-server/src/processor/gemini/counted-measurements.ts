...
import { boundedCountBody } from "./count-policy.js";
    ...
    try {
      return await bounded(async (signal) => {
        try {
          ...
          const countFirst = "countPolicy" in snapshot.runtime;
          const tokenLimit = Math.min(snapshot.policy.maxInputTokensPerRequest, analysisInputTokenCeiling(snapshot.runtime));
          const requestSignal = signal;
          const policyDeadline = "countPolicy" in snapshot.runtime ? performance.now() + snapshot.runtime.countPolicy.timeoutMs : Infinity;
          const body = "countPolicy" in snapshot.runtime ? boundedCountBody(input, snapshot.runtime.countPolicy) : JSON.stringify(buildTokenCountRequest(input));
          if (!countFirst && !o.inputBoundVerifier) unavailable();
          const bound = countFirst ? undefined : verifyGeminiInputBound({ audit,
            raw: await o.inputBoundVerifier!.inspect(structuredClone(audit), signal),
            verifier: o.inputBoundVerifier!, maxInputTokens: tokenLimit, clock, signal });
          const binding = { projectRef: scope.projectRef, inputApprovalId: scope.inputApprovalId, inputFingerprint: scope.inputFingerprint,
            requestFingerprint: audit.requestFingerprint, model: GEMINI_TEST_MODEL, promptVersion: GEMINI_PROMPT_VERSION,
            ...(countFirst ? { inputAccounting: "acceptance-allowance" as const } : {}) } as const;
          bindingHash = countBindingHash(binding);
          const guard = (lockedClock = clock) => {
            requestSignal.throwIfAborted(); permission.assertCurrent();
            if (performance.now() >= policyDeadline) unavailable();
          ...
          const sending = { ...reserve, type: "sending" as const, limits: snapshot.entitlement.providerLimits,
            notAfter: new Date(Math.min(Date.parse(snapshot.validUntil), bound ? Date.parse(bound.evidence.validUntil) : Infinity)).toISOString() };
          ...
          try {
            const launched = await o.repository.launchAnalysisCount(ticket, (lockedClock) => {
              ...
              responsePromise = (o.fetch ?? globalThis.fetch)(TOKEN_PROBE_ENDPOINT, { method: "POST", redirect: "error", signal: requestSignal,
                headers: { "content-type": "application/json", "x-goog-api-key": o.apiKey! }, body }).then((r) => {
                response = r; if (requestSignal.aborted) { cancelBody(); unavailable(); } return r;
              });
            ...
            const totalTokens = await readTokenCountResponse(reply, requestSignal);
            ...
            if (result.replayed || result.halted || record.status !== "settled" || record.bindingHash !== bindingHash ||
                ...
                totalTokens > (bound?.evidence.totalInputTokenUpperBound ?? tokenLimit) || totalTokens > tokenLimit) unavailable();
            guard(); return { totalTokens };
          } finally { requestSignal.removeEventListener("abort", cancelBody); cancelBody(); }
