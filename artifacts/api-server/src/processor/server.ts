...
export function createProcessorApp({
  ...
  // Managed API Server health contract. Keep the processor-specific /health
  // response above for existing callers while exposing the scaffold route.
  app.get("/api/healthz", (_request, response) => {
    response
      .status(readiness.ready ? 200 : 503)
      .json({ status: readiness.ready ? "ok" : "starting" });
  });
